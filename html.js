...
function htmlToPdf(stripe,UserData) {
 ...
 keys = Object.keys(stripe.metadata); 
  ...
  for(var i=0;i<List.length;i++){
    table_data = table_data + `<tr>
      ...
      <td class="subTableData">£`+ List[i].amount +`</td>
    </tr>`
    }
...
<body>
  <div id='pdf' style="background: #f5f0c4; margin: auto; min-width: 55%; width: 80%; border: 1px solid black;  ">
  ...
  <table style=" width: 80%; margin: auto; border-top : 2px solid;">
   <tr><div style="background: #eee; padding-left: 7px; padding:5px; 	border: 1px solid #999;">Contribution Item Information </div></tr>
    ...
    <tr class="halfTwo">
      <td class="halfFirst" style="width:50%">First Name(s)</td>
...
pdf.create(html).toFile('./document.pdf',function(err,stream){
  if(err){
      console.log(err);
  }
  else{
